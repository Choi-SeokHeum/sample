    ...
    while ( ( dir = readdir( d ) ) ) {
        if ( strstr( dir->d_name, ".jwt" ) ) {
            ...
            if( rviValidateCredential( handle, cred, cert ) == RVI_OK ) {
                printf("chltjrgma 151515151515151515151515\n");
...
char *rviGetPubkeyFile( char *filename )
{
    ...
    printf("chltjrgmachltjrgma 1111111111111111111111\n");
    if( !certbio ) { ret = ENOMEM; goto exit; }
    /* Then read the certificate from the string */
    printf("chltjrgmachltjrgma 22222222222222222222\n");
    cert = PEM_read_bio_X509( certbio, NULL, 0, NULL );
    printf("chltjrgmachltjrgma 333333333333333333333\n");
    if( !cert ) { ret = 1; goto exit; } 
    /* Get the public key from the certificate */
    printf("chltjrgmachltjrgma 4444444444444444444444\n");
    pkey = X509_get_pubkey(cert); 
    if( !pkey ) { ret = 1; goto exit; }
    /* Make a new memory BIO */
    printf("chltjrgmachltjrgma 555555555555555555555\n");
    mbio = BIO_new( BIO_s_mem() ); 
    if( !mbio ) { ret = ENOMEM; goto exit; }
    printf("chltjrgmachltjrgma 6666666666666666666666\n");
    ...
    printf("chltjrgmachltjrgma 77777777777777777777\n");
    ...
    printf("chltjrgmachltjrgma 8888888888888888888\n");
    /* Load the string into memory */
    ret = BIO_read(mbio, key, length);
    printf("chltjrgmachltjrgma 999999999999999999999\n");
    if(verbose){
    ...
    printf("chltjrgmachltjrgma 10101010101010101010101010\n");
...
int rviValidateCredential( TRviHandle handle, const char *cred, X509 *cert )
{
    ...
    printf("chltjrgma~~~~chltjrgma~~~~~~~~~~ 111111111111111111111111111\n");
    ...
    printf("chltjrgma~~~~chltjrgma~~~~~~~~~~ 2222222222222222222222222222222\n");
    if( ret ) {
        goto exit;
    }
    ...
    printf("chltjrgma~~~~chltjrgma~~~~~~~~~~ 3333333333333333333333333333333333\n");
    /* Check validity: start/stop */
    time(&rawtime);
    validity = rviGetJsonGrant( jwt, "validity" );

    printf("chltjrgma~~~~chltjrgma~~~~~~~~~~ 44444444444444444444444444444444444444444\n");
    int start = json_integer_value( json_object_get( validity, "start" ) );
    int stop = json_integer_value( json_object_get( validity, "stop" ) );

    if( ( start > rawtime ) || ( stop < rawtime ) ) { ret = -1; goto exit; }

    printf("chltjrgma~~~~chltjrgma~~~~~~~~~~ 5555555555555555555555555555555555\n");
    ...
    printf("chltjrgma~~~~chltjrgma~~~~~~~~~~ 666666666666666666666666666666666666666\n");
    sprintf(tmp, "%s%s%s", certHead, deviceCert, certFoot);

    /* Check that certificate in credential matches expected cert */
    bio = BIO_new( BIO_s_mem() );

    printf("chltjrgma~~~~chltjrgma~~~~~~~~~~ 77777777777777777777777777\n");
    if(verbose){
    ...
    printf("chltjrgma~~~~chltjrgma~~~~~~~~~~ 888888888888888888888888888888888\n");
